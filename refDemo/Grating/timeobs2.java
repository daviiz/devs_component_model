...
/*
This model timeobs2 accepts L (length of stretch which is by default put as 70) and pin
...
public class timeobs2 extends ViewableAtomic{
...
public timeobs2(String name){
   super(name);
   addInport("V2");
   addOutport("tout");

   addTestInput("V2",new doubleEnt(0));
   addTestInput("V2",new doubleEnt(4));
}
...
Continue(e);
   ...
   if (phaseIs("passive")) {
     for (int i=0; i< x.getLength();i++)
       if (messageOnPort(x,"V2",i)){
         entity val = x.getValOnPort("V2", i);
...
public message  out(){
    ...
    if (phaseIs("respond")&& flag) {
      ...
      m.add(con);
      flag = false; //After once the ouput is nt given
    }
    return m;
}
