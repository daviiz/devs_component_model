...
public class r6controller extends singlerobot {
	...
	public r6controller(String name) {
		super(name);
	}
	...
	public void deltint() {
		if (phaseIs("output"))
			...
			change = false;
			if (phaseIs("line")) {
				position.x = positionin.x - 5 * velocity.x * L / v;
				position.y = positionin.y - 5 * velocity.y * L / v;
				holdIn("output", 5 * T / 100);
			} else if (phaseIs("block")) {
				position.x = positionin.x + velocity.y * L / v - 2 * velocity.x * L / v;
				position.y = positionin.y - velocity.x * L / v - 2 * velocity.y * L / v;
				holdIn("output", 5 * T / 100);
			} else {
				position.x = positionin.x - Math.sqrt(3) * velocity.x * L / v;
				position.y = positionin.y - Math.sqrt(3) * velocity.y * L / v;
				holdIn("output", 5 * T / 100);
			}
		} else {
