...
public class r2 extends singlerobot {
	...
	public void deltint() {
		if (phaseIs("output"))
			...
			change = false;
			if (phaseIs("line")) {
				position.x = positionin.x - velocity.x * L / v;
				position.y = positionin.y - velocity.y * L / v;
				holdIn("output", T / 100);
			} else if (phaseIs("block")) {
				position.x = positionin.x + velocity.y * L / v;
				position.y = positionin.y - velocity.x * L / v;
				holdIn("output", T / 100);
			} else {
				position.x = positionin.x - Math.sqrt(3) * velocity.x * L / (2 * v) + velocity.y * L / (2 * v);
				position.y = positionin.y - Math.sqrt(3) * velocity.y * L / (2 * v) - velocity.x * L / (2 * v);
				holdIn("output", T / 100);
			}
		} else {
