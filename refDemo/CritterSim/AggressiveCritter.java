 ...
 * Home Page >/A>
 *
 * AggressiveCritter extends Critter.  The AggressiveCritter moves around the board, eats other
 * critters and reproduces.  It does not eat vegetation.
 */
public class AggressiveCritter extends Critter{
	...
	public AggressiveCritter() {
		this("AggressiveCritter");
	}

	public AggressiveCritter(String name) {
		this(name, 0,0,1,1);
	}

	public AggressiveCritter(String name, int x, int y, int width, int height) {
		super(name, x, y, width, height);
	}

	public AggressiveCritter(String name, int x, int y, int width, int height, long randomSeed) {
	...
	protected void setNextPhase(){
		...
		if(eatTimer >= minFeedInterval){
			...
			String food = preyLocation();
			if(food != null){
				//Store the food location
				eatDir = food;
				sending.add(makeContent(eatDir, new CritterEvent(this, CritterEvent.ATTACKING)));
				holdIn(EATING,0);
				return;
			}
		}//end if eat
	...
	protected CritterEvent spawnEvent(String dir){
		...
		AggressiveCritter crit = new AggressiveCritter(	name + "." + ++spawnCount,
														newx, newy,
														width, height,
														rand.nextLong());
		crit.initialize();
...
//			case CritterEvent.MIGRATE_DENIED:
	...
	String preyLocation(){
		for(int i = 0; i < 3 ; i++){
			for(int j = 0; j < 3; j++){
				if(	(map[i][j] instanceof Critter) &&
					(! (map[i][j] instanceof Vegetation))  &&
					(! (map[i][j] instanceof AggressiveCritter))&&
                                        ! ((PassiveCritter)map[i][j]).getPhase()
                                           .equals(PassiveCritter.SPAWNING)
                                        )
					{ return outStr(i,j); }
			}//end inner for
		}//end outer for
	...
	//Converts the row and column into the corresponding out port name
	protected String outStr(int col, int row){
		if(row == 0  && col == 0)	return outNW;
		if(row == 0  && col == 1)	return outN;
		if(row == 0  && col == 2)	return outNE;
		if(row == 1  && col == 0)	return outW;
		if(row == 1  && col == 2)	return outE;
		if(row == 2  && col == 0)	return outSW;
		if(row == 2  && col == 1)	return outS;
		if(row == 2  && col == 2)	return outSE;
		return null;
	}
